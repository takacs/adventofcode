...
[1518-06-01 00:47] wakes up""".strip().replace('[', '').split('\n')

import datetime
from collections import Counter

split = [ [datetime.datetime.strptime(line.split(']')[0], "%Y-%m-%d %H:%M"), line.split(']')[1][1:]] for line in inp ]
split.sort(key=lambda x: x[0])

def sleepy_guard(gdict, ssch):

    for line in ssch:
        l = line[1].split(' ')
        if l[0] == 'Guard':
            curr_guard = int(l[1][1:])
        if l[0] == 'falls':
            sleep_start = line[0]
        if l[0] == 'wakes':
            sleep_end = line[0]
            try:
                gdict[curr_guard][0] += sleep_end-sleep_start
                while(sleep_start != sleep_end):
                    gdict[curr_guard][1].append(sleep_start.minute)
...
def sleepiest_min(gdict):
    sleepmin = {}
    mostg, mostm, c = 0,0,0
    for key, value in gdict.items():
        stats = Counter(gdict[key][1])
        ck = max(stats.keys(), key=(lambda key: stats[key]))
        print(stats[ck], c, mostm, mostg)
        if stats[ck] >= c:
            c = stats[ck]
            mostg = key
            mostm = ck
...
gdict = {}
gdict = sleepy_guard(gdict, split)
sleepiest_guard = max(gdict.keys(), key=(lambda key: gdict[key][0]))
print(sleepiest_guard, Counter(gdict[sleepiest_guard][1]))

sleepiest = sleepiest_min(gdict)
